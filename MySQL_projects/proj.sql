...
-- ## 3.1.1 Total sales per months throughout the period
CREATE VIEW total_sales_monthly AS
...
-- ## 3.1.2 The summary of total sales per weekday
CREATE VIEW total_sales_per_weekday AS
SELECT DATE_FORMAT(date, '%W') AS weekday, 
	SUM(total_price_per_unit) AS total_sales
FROM sales
GROUP BY weekday;

-- ## 3.1.3 Total per weekday throughout the given period
CREATE VIEW total_sales_daily AS
SELECT YEAR(DATE) AS year, 
	MONTH(date) AS month, 
    ...
    SUM(total_price_per_unit) AS total_sales
FROM sales
GROUP BY year, month, day, weekday
ORDER BY year, month, day, weekday;

-- ### 3.2 Time
-- ## 3.2.1 Summary of total sales made per hour
CREATE VIEW total_sales_quant_slcnt_per_hour AS
SELECT HOUR(time) AS hour, 
	SUM(total_price_per_unit) AS total_sales, 
    SUM(quantity) AS total_quntity, 
    COUNT(ticket_number) AS number_of_customers
FROM sales
GROUP BY hour
ORDER BY hour;

--  ## 3.2.2 Summary of total sales made per hour on each weekday
CREATE VIEW total_sales_salcnt_per_weekday_hourly AS
SELECT DATE_FORMAT(date, '%W') AS weekday, 
	HOUR(time) AS hour, 
    SUM(total_price_per_unit) AS total_sales, 
    COUNT(ticket_number) AS total_sales_count
FROM sales
GROUP BY weekday, hour
ORDER BY weekday, hour;

-- ########## EXPERIMENT. Total sales per hour per day throughout the entire period
CREATE VIEW EXP_total_sales_daily_hourly_all AS
SELECT YEAR(DATE) AS year, 
	MONTH(date) AS month, 
    DATE_FORMAT(date, '%d') AS day, 
    DATE_FORMAT(date, '%W') AS weekday, 
...
FROM sales
GROUP BY year, month, day, weekday, hour
ORDER BY year, month, day, weekday, hour;
-- ##########

-- ###### 4. Products analysis ######

-- ### 4.1 Top 10 sold products for the entire period
...
-- ### 4.2 Performance of top 10 products broken down by month throughout the period
CREATE VIEW top10_tot_sal_monthly AS
SELECT YEAR(date) AS year,
	MONTH(date) AS month,
	article, 
	SUM(total_price_per_unit) AS total_sales
...
WHERE article IN (
	SELECT article
	FROM top10_total_sales_article -- this is the view created in section 4.1
)
GROUP BY year, month, article
ORDER BY year, month, total_sales DESC;

-- ### EXPERIMENT: Top 10 sold articles every month ###
CREATE VIEW top10_montly_sales AS
WITH ranked_montly_sales AS (
SELECT YEAR(date) AS year,
	MONTH(date) as month,
    article,
	SUM(total_price_per_unit) AS total_sales,
    ROW_NUMBER() OVER (PARTITION BY YEAR(date), MONTH(date) ORDER BY SUM(total_price_per_unit) DESC) AS prod_rank
FROM sales
GROUP BY year, month, article
ORDER BY year, month, total_sales DESC
)
SELECT year, month, article, total_sales
FROM ranked_montly_sales
WHERE prod_rank <= 10
ORDER BY year, month, total_sales DESC
;
-- ###########################

-- ### 4.3 Top 10 sold products in terms of the quantity sold
